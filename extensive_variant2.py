...
class ProductionPlanningProblem:
    ...
    def __init__(self,n_products:int, n_facilities:int, n_production_levels:int, n_locations:int,instance_no:int=1):
      ...
      The instance number acts as the seed'
      for the random number generator (default 1).
...
for i in range(len(seed_vec)): 
    ...
    #10 products, 5 facilities and 3 production levels, 4 demand locations
    pp = ProductionPlanningProblem(10,5,3,4, seed_vec[i])
    ...
    min_price = min(P[p] for p in range(n_products))

    H = {(f, q): r.uniform(0.1 * min_price, 0.9 * min_price)
     for f in range(n_facilities)
     for q in range(n_locations)}
    
    Y, D = pp.generate_sample(scenario_vec[0],seed_vec[i])
    ...
    w = m.addVars(n_products, n_facilities, n_locations, S, lb=0.0)
    o = m.addVars(n_products, n_facilities, S, lb=0.0)
    ...
    m.addConstrs(
    gp.quicksum(w[p,f,q,s] for q in range(n_locations)) + o[p,f,s] == Y[p,f,s] * x[p,f]
    ...
    m.addConstrs(
    gp.quicksum(w[p,f,q,s] for f in range(n_facilities)) <= D[p,q,s]
    for p in range(n_products)
    for q in range(n_locations)
    for s in S
)
